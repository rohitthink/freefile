...
class SBIParser(BankStatementParser):
    ...
    HEADER_KEYWORDS = ["txn date", "value date", "description", "debit", "credit", "balance"]
    ...
    def _find_header_row(self, table: list) -> Optional[int]:
        for i, row in enumerate(table):
            ...
            if ("txn" in row_text or "date" in row_text) and ("debit" in row_text or "credit" in row_text):
                return i
        return None
    ...
    def _map_columns(self, headers: list[str]) -> Optional[dict]:
        ...
        for i, h in enumerate(headers):
            if ("txn" in h or "trans" in h) and "date" in h:
                col_map["date"] = i
