...
class HDFCParser(BankStatementParser):
    ...
    HEADER_KEYWORDS = ["narration", "withdrawal", "deposit", "closing balance"]
    ...
    def _find_header_row(self, table: list) -> Optional[int]:
        for i, row in enumerate(table):
            ...
            if "narration" in row_text and ("withdrawal" in row_text or "deposit" in row_text):
                return i
        return None
    ...
    def _map_columns(self, headers: list[str]) -> Optional[dict]:
        ...
        for i, h in enumerate(headers):
            if "date" in h and "value" not in h and "date" not in col_map:
                col_map["date"] = i
                ...
                col_map["credit"] = i
            elif "closing" in h or "balance" in h:
                col_map["balance"] = i
